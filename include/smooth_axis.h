...
//     smooth_axis_update_raw(&axis, raw);
...
//     uint16_t raw    = read_adc();
//     smooth_axis_update_raw_dt(&axis, raw, dt_sec);
//     uint16_t clean  = smooth_axis_get_u16_full(&axis);
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
typedef enum {
    // Use smooth_axis_update_raw_dt()
    SMOOTH_AXIS_MODE_LIVE_DT = 0,

    // Use smooth_axis_update_raw(), dt inferred during warm-up
...
typedef struct {
    ...
    //
    // Choose ONE:
    //   - AUTO_DT:  smooth_axis_default_config_auto() + smooth_axis_update_raw()
    //   - LIVE_DT:  smooth_axis_default_config_live_dt() + smooth_axis_update_raw_dt()
    //
